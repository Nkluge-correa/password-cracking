        ...
        dbc.Modal(
            [
                ...
                dbc.ModalBody([dcc.Markdown('''
                                ...
                                ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('''
                                A **cryptographic hash function** (**CHF**) is a mathematical [algorithm](https://en.wikipedia.org/wiki/Algorithm "Algorithm") that [maps](https://en.wikipedia.org/wiki/Map_(mathematics) "Map (mathematics)") data of an arbitrary size (often called the "message") to a [bit array](https://en.wikipedia.org/wiki/Bit_array "Bit array") of a fixed size (the "[hash value](https://en.wikipedia.org/wiki/Hash_value "Hash value")", or "digest"). It is a [one-way function](https://en.wikipedia.org/wiki/One-way_function "One-way function"), that is, a function for which it is practically infeasible to invert or reverse the computation. This also means that there can be **data-loss** during the hashing process.
                                ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('### **SHA-1**'), html.Br(),
                               dcc.Markdown('___'),
                               dcc.Markdown('''
                                ```
                               ...
                               dcc.Markdown('''
                                **SHA-1** (**Secure Hash Algorithm 1**) is a cryptographically **broken** but still widely used [hash function](https://en.wikipedia.org/wiki/Hash_function "Hash function") which takes an input and produces a 160-bit (20-byte) hash value known as a [message digest](https://en.wikipedia.org/wiki/Message_digest "Message digest") – typically rendered as a [hexadecimal](https://en.wikipedia.org/wiki/Hexadecimal "Hexadecimal") number, 40 digits long. It was designed by the United States [National Security Agency](https://en.wikipedia.org/wiki/National_Security_Agency "National Security Agency"), and is a U.S. [Federal Information Processing Standard](https://en.wikipedia.org/wiki/Federal_Information_Processing_Standard "Federal Information Processing Standard").
                                ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('''
                                 *In this example of attack, we are trying to break a SHA-1 hash, as a mere example on password safety* (**Do not do this at home kids!**).
                                 ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('### **Leaks ☣️**'), html.Br(),
                               dcc.Markdown('___'),
                               dcc.Markdown('''
                                ...
                                ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('''
                                This leak, and many others, help attackers create *"dictionary's"* that can be use to help password-cracking. For example, the [RockYou2021](https://github.com/ohmybahgosh/RockYou2021.txt) has **100 GB** of plain leaked passwords ready to be used in [dictionary attacks](https://en.wikipedia.org/wiki/Dictionary_attack).
                                ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('''
            ...
            [
                ...
                dbc.ModalBody([dcc.Markdown('''
                               ...
                               dcc.Markdown('''
                                ...
                                ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('''
                                 *"I do not believe you are cracking these passwords! You are storing these words whiteout hashing! The cake is a lie!"*
                                 ''', style={'font-size': 24, 'text-align': 'center'}), html.Br(),
                               dcc.Markdown('''
    ...
    [
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalBody([dcc.Markdown('''
                                Here are a couple of basic instructions on how to create a *"not-super-easy-to-break"* password:
                                ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('''
                                ...
                                ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('### Password Entropy', style={
                                            'font-size': 24}), html.Br(),
                               dcc.Markdown('___'),
                               dcc.Markdown('''
                               ...
                               dcc.Markdown('''
                                ...
                                ''', style={'font-size': 24}), html.Br(),
                               dcc.Markdown('''
                                Instead of the number of guesses needed to find the password with certainty, the [base-2 logarithm](https://en.wikipedia.org/wiki/Binary_logarithm "Binary logarithm") of that number is given, which is commonly referred to as the number of "entropy bits" in a password. A password with an entropy of 42 bits calculated in this way would be as strong as a string of 42 bits chosen randomly. Put another way, a password with an entropy of 42 bits would require 242 (4,398,046,511,104) attempts to exhaust all possibilities during a [brute force search](https://en.wikipedia.org/wiki/Brute_force_search "Brute force search"). Thus, increasing the entropy of the password by one bit doubles the number of guesses required, making an attacker's task twice as difficult. On average, ***an attacker will have to try half the possible number of passwords before finding the correct one.***
                                ''', style={'font-size': 24}), html.Br(),
                               ...
                               dbc.Label(dcc.Markdown(
                                   '*Check your password strength bellow!*', style={'font-size': 24})),
        ...
        html.Div([dbc.FormText(dcc.Markdown("*Choose the number of words in your password (>= 4 is recommended)*",
                 style={'margin-left': '15px', }))], style={'display': 'inline-block', 'text-align': 'right'})
    ], style={
        'margin-left': '15px',
        'margin-right': '150px',
...
password_input = html.Div(
    [
        ...
        dbc.FormText(dcc.Markdown("What about *password101*",
...
hash_input = html.Div(
    [
        ...
        dbc.FormText(dcc.Markdown(
            "What about *523cf99e800d57d0ff0ac7b97e04ebc2b9b4b263*", style={'margin-left': '15px'})),
    ]
)
...
app.layout = dbc.Container(
    fluid=True,
    children=[
        html.H1('Dictionary Attack & Password Generator 👾🔒', style={
            'font-style': 'bold',
            'margin-top': '15px',
            'margin-left': '15px',
...
def password_entropy(value):
    if len(value) <= 1:
        ...
        return f'''
                ...
                To guess this password, character-by-character, via brute-force, it would take up to:\n
                2 ** ({round(len(text) * math.log2(x), 2)}) = {'{:,.2f}'.format(int(2 ** (round(len(text) * math.log2(x), 2))))} guesses.\n
...
def generate_password(click, value):
    if click > 0:
        ...
        To guess this password, character-by-character, via brute-force, it would take up to:\n
        2 ** ({round(len(password_len) * math.log2(x), 2)}) = {'{:,.2f}'.format(int(2 ** (round(len(password_len) * math.log2(x), 2))))} guesses.\n
